...
config = {
  function: {
    ...
    reorder: {
      // Reverse word order
      cdab: (buffer) => {
        hexstr = buffer.toString('hex');
        while(hexstr.length % 4 != 0) {
          hexstr = `0${hexstr}`;
        }
        // console.log(`hex: ${hexstr}`);
        hexarr = hexstr.match(/.{1,4}/g);
        hexstr = hexarr.reverse().join(``)
        // console.log(`reo: ${hexstr}`)
        newbuf = Buffer.from(hexstr, 'hex');
        return newbuf;
      }
    },
    bufferToDataView: (buffer) => {
      ...
      try {
        // console.log(`pre buffer: (${typeof buffer}) ${buffer.toString(`hex`)}`)
        reorderbuf = config.function.reorder.cdab(buffer)
        // console.log(`pos buffer: (${typeof reorderbuf}) ${reorderbuf.toString(`hex`)}`)
      } catch(error) {
          console.error(error)
      }
      let buf = new ArrayBuffer(reorderbuf.length);
      let view = new DataView(buf);
      reorderbuf.forEach((b, i) => {
        view.setUint8(i, b);
      });
      return view;
    },
    ...
    bufferToFloat64: (buffer) => config.function.bufferToDataView(buffer).getFloat64(),
    bufferToFloat64Div1000: (buffer) => config.function.bufferToDataView(buffer).getFloat64()/1000.0,
    bufferToFloat64Mul1000: (buffer) => config.function.bufferToDataView(buffer).getFloat64()
  },
