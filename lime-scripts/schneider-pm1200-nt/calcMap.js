...
config = {
  function: {
    ...
    littleEndianOrder: (buff) => {
      let srcArr = buff.toString(`hex`).split('');
      let arr = [];
      for(let i = 0; i < srcArr.length/4;i++) {
        let word = ``;
        for(let j = 0; j < 4;j++)
        word = `${word}${srcArr[i*4+j]}`;
        arr.push(word);
      }
      return Buffer.from(arr.reverse().join(``), `hex`);
    },
    bufferToDataView: (buffer) => {
      // console.log(`buffer:`, buffer);
      buffer = config.function.littleEndianOrder(buffer);
      // console.log(`buffer:`, buffer);
      let buf = new ArrayBuffer(buffer.length);
      let view = new DataView(buf);
      // let view = new DataView(config.function.littleEndianOrder(buf));
      buffer.forEach((b, i) => {
        view.setUint8(i, b);
      });
      return view;
    ...
    bufferToFloat32Div1000: (buffer) => {
      try {
        let result = Number(config.function.bufferToDataView(buffer).getFloat32());
        result = (typeof result == `number` && !isNaN(result)) ? result/1000.0 : 0.0;
        // console.log(`buffer result: ${result}`);
        return result;
      } catch(err) {
        return 0.0;
